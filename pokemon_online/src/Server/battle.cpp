...
void BattleSituation::testCritical(int player, int target)
{
    ...
    // Modified: critical hits never happen
    bool critical = false;
...
int BattleSituation::calculateDamage(int p, int t)
{
    ...
    // Modified: Always selects a damage amount in the middle of the random damage range
    int randnum = 92;
    ...
    if (battleMemory().contains(sport) && pokeMemory(battleMemory()[sport].toInt()).value(sport).toBool()) {
        // Modified: Sports now completely nullify the type being sported
        power = 0;
    }
